...
public class ParamsUtil {
     ...
     * @return
     */
    public static boolean checkDcode(String... codes) {
        for (int i = 0; i < codes.length; i++) {
            if (!codes[i].matches("[0-9]{6}")) {
                return false;
            }
        }
        if (!codes[0].substring(0, 2).equals(codes[1].substring(0, 2))
                || !codes[0].substring(0, 2).equals(codes[2].substring(0, 2))
                || !codes[1].substring(2, 4).equals(codes[2].substring(2, 4))) {
            return false;
        }
        return true;
    }
     ...
     * @return
     */
    public static Map<String, Object> rtnPageRs(int pageNo, int pageSize, List list) {
        if (pageNo <= 0) {
            pageNo = 1;
        }
        if (pageSize <= 0) {
        ...
        if (Math.ceil(1.0 * total / pageSize) < pageNo) {
            json.put("total", 0);
            json.put("list", null);
        } else {
            json.put("total", list.size());
            List rtnList = rtnList((pageNo - 1) * pageSize, pageSize, list);
            json.put("list", rtnList);
        }
        rtn.put("data", json);
        return rtn;
