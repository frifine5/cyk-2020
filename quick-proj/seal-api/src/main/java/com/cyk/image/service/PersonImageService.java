...
import com.cyk.common.PsaImageUtil;
...
public class PersonImageService {
     ...
     * 方章：固名缩比 196像素, 2-4个字
     * */
    public byte[] genPsSquareSeal(String name, float ri, Color fontColor){
        ...
        if(name.length()<2 || name.length()>4){
            throw new IllegalArgumentException("名字超长(4)");
        }
     ...
     * 方章：固名缩比 196像素, 5-6个字
     * */
    public byte[] genPsSquareSealSix(String name, float ri, Color fontColor){
        int canvasWidth = 196;// 正方形以宽为准
        int canvasHeight = 196;
        int broad = 5;
        if(ParamsUtil.checkNull(name)){
     ...
     * */
    public byte[] genPsRectSealLonger(String name, float ri, float wh, Color fontColor){
        int fontSize = 52;
